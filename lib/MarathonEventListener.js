...
function MarathonEventListener (options) {
    ...
    if (options.labels && options.labels.length > 0) {
        self.options.labels = [];
            ...
            self.options.labels.push(label);
    ...
    self.logger.info(self.options.labels)
...
MarathonEventListener.prototype.handleEvent = function (event) {
    ...
    function parseEvents (event) {
        ...
        self.logger.info("Original event: " + JSON.stringify(event));
            ...
            case "deployment_info":
                ...
                event.data.plan.steps.forEach(function (step) {
                    step.actions.forEach(function (action) {
                        ...
                        self.logger.info("Event: " + JSON.stringify(action.app));

                        
                        /** Find details about the App **/
                        var app_labels = {};
                        var app_label_str = "";
                        var target_apps = event.data.plan.target.apps;
                        var target_groups = event.data.plan.target.groups;

                        event.data.plan.target.groups.forEach(function(group) {

                            group.groups.forEach(function(innergroup) {

                             innergroup.apps.forEach(function(app) {

                             if (app.id == action.app) {
                                 app_labels = app.labels;
                                 self.logger.info("Event:### " + JSON.stringify(app.id));
                                
                                 self.logger.info("Value:### " + self.options.labels);

                                 for (var label in app.labels) {
                                     self.logger.info("label : "+ label);
                                     self.logger.info(self.options.labels)
                                     self.logger.info(typeof self.options.labels.indexOf(label));
                                    if (self.options.labels.indexOf(label) >= 0) {
                                        self.logger.info("Event:### " + JSON.stringify(label) + " : " + JSON.stringify(app.labels[label]));
                                        app_labels = app.labels;
                                        app_label_str += " " + label + " is " + app.labels[label]+"\n\n";
                                    }
                                 }
                                 app_label_str = app_label_str.substring(0, app_label_str.length);
                                 }
                              });
                            });
                        ...
                        event.data.plan.target.apps.forEach(function(app) {
                            if (app.id == action.app) {
                                 app_labels = app.labels;
                                 self.logger.info("Event:### " + JSON.stringify(app.id));
                                 ...
                                 for (var label in app.labels) {
                                    ...
                                    if (self.options.labels.indexOf(label) >= 0) {
                                        self.logger.info("Event:### " + JSON.stringify(label) + " : " + JSON.stringify(app.labels[label]));
                                        app_labels = app.labels;
                                        app_label_str += " " + JSON.stringify(label) + " is " + JSON.stringify(app.labels[label])+"\n\n";
                                    }
                                 }
                                 app_label_str = app_label_str.substring(0, app_label_str.length);
                            }
                        ...
                        attachment.footer = app_label_str;
                        i++;
                    });
                });
